...
export default function CreateQuiz() {
  ...
  const saveQuiz = async (e) => {
    ...
    const { data: quiz, error: quizError } = await supabase
      .from('quiz_content')
      .insert([{ title, description, category_id: categoryId, difficulty, time_limit_per_question: timeLimit, type_id: typeId, creator_id: user.id }])
      .select()
      .single()
    ...
    for (const [qIndex, q] of questions.entries()) {
      const { data: question, error: qErr } = await supabase
        .from('quiz_questions')
        .insert([{ quiz_id: quiz.id, question_text: q.text, position: qIndex + 1 }])
        .select()
        .single()
